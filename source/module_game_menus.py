       ...
       [
        (try_for_range, ":npc", companions_begin, companions_end),
          ...
          (else_try),
            ...
            (str_store_string, s96, "@Unknown"),
            (try_begin),
              (ge, reg6, 0),
              (str_store_party_name, s96, reg6),
            (try_end),
            (troop_get_slot, reg7, ":npc", slot_troop_prisoner_of_party),
            (display_message, "@{!}{s4}: occ={reg3}, met={reg4}, PPH={reg5}, ctr={s96}, PoP={reg7}"),
          (try_end),
        (try_end),
        ]
       ),
    ...
    [
      ("build_siege_tower_cont",[],
       "Start building.", [
           ...
           (val_mul, ":hours_takes", 14),
           (val_div, ":hours_takes", 3),
           (val_add, ":hours_takes", 12),
           (store_add, "$g_siege_method_finish_hours",":cur_hours", ":hours_takes"),
           (assign,"$auto_besiege_town","$current_town"),
  ...
  (
    ...
    [
        ...
        (try_begin),
          ...
          (val_mul, "$qst_eliminate_bandits_infesting_village_num_bandits", 150),
          (val_div, "$qst_eliminate_bandits_infesting_village_num_bandits", 100),
          (val_add, "$qst_eliminate_bandits_infesting_village_num_bandits", 10),
    ...
    [
      ("continue",[],"Continue...",
       [(party_set_slot, "$g_encountered_party", slot_village_infested_by_bandits, 0),
        ...
        (party_get_slot, ":num_cattle", "$current_town", slot_center_head_cattle),
        (val_div, ":num_cattle", 2),
        (val_max, ":num_cattle", 0),
        (party_set_slot, "$current_town", slot_center_head_cattle, ":num_cattle"),
        (party_get_slot, ":num_sheep", "$current_town", slot_center_head_sheep),
        (val_div, ":num_sheep", 2),
        (val_max, ":num_sheep", 0),
        (party_set_slot, "$current_town", slot_center_head_sheep, ":num_sheep"),
 ...
 The villagers have little left in the way of wealth after their ordeal, but they offer you {reg10?all they can find:a few heads of cattle}.",
    ...
    [(party_get_slot, ":bandit_troop", "$g_encountered_party", slot_village_infested_by_bandits),
     ...
     (try_begin),
       ...
       (call_script, "script_end_quest", "qst_eliminate_bandits_infesting_village"),
       #Add quest reward
       (call_script, "script_change_player_relation_with_center", "$g_encountered_party", 7),
...
##          ]),
	...
	[
	 ...
	 ##nested diplomacy end+
      ...
      ("town_cheat", [(ge, "$cheat_mode", DPLMC_CHEAT_YES),],
      "Use cheats.",
      [(jump_to_menu, "mnu_town_cheats"),
      ]),
...
# They must learn field discipline and the steadiness to follow orders in combat before they can be thought to use arms.",
    ...
    [
      ("peasants_against_bandits_attack_resist", [], "Prepare for a fight!",
       [ ## SB : use new bandit script
        ...
        (store_character_level, ":level", "trp_player"),
        (val_div, ":level", 3),
        (store_add, ":min_bandits", ":level", 10),
 ...
 but they offer you all they can find to show their gratitude.",
  ...
  (
    ...
    (assign, ":distance_center_castles_count", 0),
    (assign, ":distance_center_castles_max", 0),
    (assign, ":distance_center_castles_avg", 0),
    (assign, ":distance_center_castles_min", 99999),
    (assign, ":distance_center_towns_count", 0),
    (assign, ":distance_center_towns_max", 0),
    (assign, ":distance_center_towns_avg", 0),
    (assign, ":distance_center_towns_min", 99999),
    (assign, ":distance_center_villages_count", 0),
    (assign, ":distance_center_villages_max", 0),
    ...
    (try_for_range, ":center", centers_begin, centers_end),
      ...
      (try_begin),
        ...
        (val_add, ":distance_center_castles_count", 1),
        (val_add, ":distance_center_castles_avg", ":center_distance"),
        (try_begin),
          (gt, ":distance_center_castles_min", ":center_distance"),
          (assign, ":distance_center_castles_min", ":center_distance"),
        (try_end),
        (try_begin),
          (lt, ":distance_center_castles_max", ":center_distance"),
          (assign, ":distance_center_castles_max", ":center_distance"),
        (try_end),
      (else_try),
        (is_between, ":center", towns_begin, towns_end),
        (val_add, ":distance_center_towns_count", 1),
        (val_add, ":distance_center_towns_avg", ":center_distance"),
        (try_begin),
          (gt, ":distance_center_towns_min", ":center_distance"),
          (assign, ":distance_center_towns_min", ":center_distance"),
        (try_end),
        (try_begin),
          (lt, ":distance_center_towns_max", ":center_distance"),
          (assign, ":distance_center_towns_max", ":center_distance"),
        (try_end),
      (else_try),
        (is_between, ":center", villages_begin, villages_end),
        (val_add, ":distance_center_villages_count", 1),
        (val_add, ":distance_center_villages_avg", ":center_distance"),
        (try_begin),
          (gt, ":distance_center_villages_min", ":center_distance"),
          (assign, ":distance_center_villages_min", ":center_distance"),
        (try_end),
        (try_begin),
          (lt, ":distance_center_villages_max", ":center_distance"),
          (assign, ":distance_center_villages_max", ":center_distance"),
        (try_end),
      (try_end),
    (try_end),
    (val_div, ":distance_center_castles_avg", ":distance_center_castles_count"),
    (val_div, ":distance_center_towns_avg", ":distance_center_towns_count"),
    (val_div, ":distance_center_villages_avg", ":distance_center_villages_count"),
    (assign, reg80, ":distance_center_castles_count"),
    (assign, reg81, ":distance_center_castles_min"),
    (assign, reg82, ":distance_center_castles_avg"),
    (assign, reg83, ":distance_center_castles_max"),
    (str_store_string, s85, "@Castles ({reg80}) {reg81}/{reg82}/{reg83}"),
    (assign, reg80, ":distance_center_towns_count"),
    (assign, reg81, ":distance_center_towns_min"),
    (assign, reg82, ":distance_center_towns_avg"),
    (assign, reg83, ":distance_center_towns_max"),
    (str_store_string, s86, "@Towns ({reg80}) {reg81}/{reg82}/{reg83}"),
    (assign, reg80, ":distance_center_villages_count"),
    (assign, reg81, ":distance_center_villages_min"),
    (assign, reg82, ":distance_center_villages_avg"),
    ...
    (try_begin),
      ...
      (store_sub, reg90, ":cur_hours", ":last_returned"),
      ...
      (store_sub, reg91, ":cur_hours", ":last_arrival"),
      ...
      (store_sub, reg92, ":cur_hours", ":last_attacked_time"),
      (val_div, reg92, 24),

      (party_get_slot, ":market_town_no", "$g_encountered_party", slot_village_market_town),
      (str_store_party_name, s90, ":market_town_no"),
      (str_store_string, s99, "@^Market Town: {s90}^Last farmer arrival: {reg90} hours^Last farmer returned: {reg91} hours^Last attacked: {reg92} days"),
    (else_try),
      (is_between, "$g_encountered_party", towns_begin, towns_end),
      ...
      (try_end),
      (assign, reg90, ":village_count"),
      (str_store_string, s99, "@^Market Villages ({reg90}):^{s51}"),
    (try_end),
    ...
    [
      ...
      ("to_center_reports_regional_market", [], "Show regional market prices.", [
         (jump_to_menu, "mnu_center_reports_regional_market"),
      ]),

      ("to_center_reports_global_market", [], "Show global market prices.", [
  ...
  [
    ...
    (try_begin),
      ...
      (str_store_string, s29, "@^Market Town: {s50}"),
    (else_try),
      (is_between, "$g_encountered_party", towns_begin, towns_end),
      # build list of villages that use this town as their market
      (assign, ":village_count", 0),
  ...
  [
    ...
    (try_for_range_backwards, ":cur_good", trade_goods_begin, trade_goods_end),
      ...
      (try_begin), # towns
        ...
        (try_for_range, ":village_no", villages_begin, villages_end),
          ...
          (party_get_slot, ":village_price", ":village_no", ":cur_good_price_slot"),
          (val_add, ":village_price_avg", ":village_price"),
          (try_begin),
            (lt, ":village_price", ":village_price_min"),
            (assign, ":village_price_min", ":village_price"),
          (try_end),
          (try_begin),
            (gt, ":village_price", ":village_price_max"),
            (assign, ":village_price_max", ":village_price"),
          (try_end),
          (val_add, ":village_count", 1),
        (try_end),
        (try_begin),
    ...
    (try_begin),
      ...
      (party_get_slot, ":market_town_no", "$g_encountered_party", slot_village_market_town),
      (str_store_party_name, s50, ":market_town_no"),
      (str_store_string, s99, "@^Market Town: {s50}^^Prices at village (and market town):"),
    (else_try),
      (is_between, "$g_encountered_party", towns_begin, towns_end),
      # build list of villages that use this town as their market
      (assign, ":village_count", 0),
  ...
  (
    "center_reports_regional_market",0,
    "Regional Prices:^{s99}^{s90}",
    "none",
  [
    (str_clear, s50),
    (str_clear, s51),
    ...
    (try_for_range_backwards, ":cur_good", trade_goods_begin, trade_goods_end),
      ...
      # calculate prices at nearby villages or towns
      # s90 will be the result with min/avg/max or "no data"
      (try_begin), # towns
        (is_between, "$g_encountered_party", towns_begin, towns_end),
        (assign, ":town_count", 0),
        (assign, ":town_price_min", maximum_price_factor),
        (assign, ":town_price_avg", 0),
        (assign, ":town_price_max", minimum_price_factor),
        (try_for_range, ":town_no", towns_begin, towns_end),
          (neg|eq, "$g_encountered_party", ":town_no"),
          (store_distance_to_party_from_party, ":dist_to_main_party", "p_main_party", ":town_no"),
          (le, ":dist_to_main_party", ":max_reporting_distance"),
          (party_get_slot, ":town_price", ":town_no", ":cur_good_price_slot"),
          (val_add, ":town_price_avg", ":town_price"),
          (try_begin),
            (lt, ":town_price", ":town_price_min"),
            (assign, ":town_price_min", ":town_price"),
          (try_end),
          (try_begin),
            (gt, ":town_price", ":town_price_max"),
            (assign, ":town_price_max", ":town_price"),
          (try_end),
          (val_add, ":town_count", 1),
        (try_end),
        (try_begin),
          (gt, ":town_count", 0),
          (val_div, ":town_price_avg", ":town_count"),
          (assign, reg90, ":town_price_min"),
          (assign, reg91, ":town_price_avg"),
          (assign, reg92, ":town_price_max"),
          (str_store_string, s90, "@{reg90}/{reg91}/{reg92}"),
        (else_try),
          (str_store_string, s90, "@No towns"),
        (try_end),
      ...
      (else_try), # villages
        ...
        (assign, ":village_price_max", minimum_price_factor),
        (try_for_range, ":village_no", villages_begin, villages_end),
          (neg|eq, "$g_encountered_party", ":village_no"),
    ...
    (try_begin),
      ...
      (try_end),
      (assign, reg20, ":village_count"),
      (str_store_string, s99, "@^Villages ({reg20}):^{s51}^^Prices at village (min/avg/max for nearby villages):"),
    (else_try),
      (is_between, "$g_encountered_party", towns_begin, towns_end),
      (assign, ":town_count", 0),
      (try_for_range, ":town_no", towns_begin, towns_end),
        ...
        (store_distance_to_party_from_party, ":dist_to_main_party", "p_main_party", ":town_no"),
        (le, ":dist_to_main_party", ":max_reporting_distance"),
        (str_store_party_name, s50, ":town_no"),
        (try_begin),
          (eq, ":town_count", 0),
          (str_store_string_reg, s51, s50),
        (else_try),
      ...
      (str_store_string, s99, "@^Towns ({reg20}):^{s51}^^Prices at town (min/avg/max for nearby towns):"),
    (try_end),
  ],
  [("go_back_dot",[],"Go back.",[(jump_to_menu, "mnu_center_reports")])]
  ),
  ...
  (
    "center_reports_global_market",0,
    "Global Prices:^{s99}^{s90}",
    "none",
  [
    (str_clear, s50),
    (str_clear, s51),
    ...
    (try_for_range_backwards, ":cur_good", trade_goods_begin, trade_goods_end),
      ...
      # calculate prices at all villages or towns
      # s90 will be the result with min/avg/max or "no data"
      (try_begin), # towns
        (is_between, "$g_encountered_party", towns_begin, towns_end),
        (assign, ":town_count", 0),
    ...
    (try_begin),
      ...
      (try_end),
      (assign, reg20, ":village_count"),
      (str_store_string, s99, "@^Villages ({reg20}):^^Prices at village (min/avg/max for all villages):"),
    (else_try),
      (is_between, "$g_encountered_party", towns_begin, towns_end),
      (assign, ":town_count", 0),
      (try_for_range, ":town_no", towns_begin, towns_end),
...
##	(call_script, "script_merchant_road_info_to_s42", "$g_encountered_party"),
  ...
  (
    ...
    (try_for_range, ":unused", 0, ":max_iteration"),
      ...
      (assign, ":buy_price", ":random_trade_good_buy_price"),
      ...
      (try_begin),
        ...
        (try_begin),
          ...
          (assign, ":best_result_1_base", ":buy_price"),
        (else_try),
          ...
          (assign, ":best_result_2_base", ":buy_price"),
        (else_try),
          ...
          (assign, ":best_result_3_base", ":buy_price"),
        (else_try),
          ...
          (assign, ":best_result_4_base", ":buy_price"),
        (else_try),
          (eq, ":best_result_5_item", ":random_trade_good"),
          ...
          (assign, ":best_result_5_base", ":buy_price"),
        (try_end),
      (else_try),
        (try_begin),
          (gt, ":difference", ":best_result_1_profit"),
          ...
          (assign, ":best_result_5_base",   ":best_result_4_base"),
          ...
          (assign, ":best_result_4_base",   ":best_result_3_base"),
          ...
          (assign, ":best_result_3_base",   ":best_result_2_base"),
          ...
          (assign, ":best_result_2_base",   ":best_result_1_base"),
          (assign, ":best_result_1_item",   ":random_trade_good"),
          (assign, ":best_result_1_town",   ":random_town"),
          (assign, ":best_result_1_profit", ":difference"),
          (assign, ":best_result_1_base",   ":buy_price"),
    ...
    (else_try),
      ...
      (try_begin),
        ...
        (assign, reg7, ":best_result_5_base"),
        (str_store_item_name, s4, ":best_result_5_item"),
        (str_store_party_name, s5, ":best_result_5_town"),
        (str_store_string, s3, "@^Buying {s4} here for {reg7} denars and selling it at {s5} would bring a profit of {reg6} denars per item.{s3}"),
      (try_end),
      (try_begin),
        (ge, ":best_result_4_item", 0),
        (assign, reg6, ":best_result_4_profit"),
        (assign, reg7, ":best_result_4_base"),
      ...
      (try_begin),
        ...
        (assign, reg7, ":best_result_3_base"),
      ...
      (try_begin),
        ...
        (assign, reg7, ":best_result_2_base"),
      ...
      (try_begin),
        ...
        (assign, reg7, ":best_result_1_base"),
